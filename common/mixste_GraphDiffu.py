...
# Neighborhood Relationship Interactor
args = parse_args()
...
def get_spatial_graph(self_link, inward, outward):
    # Gets a list of edges and returns
    I = self_link  # self-connection
    In = inward     # Inside edge
    Out = outward   # Outgoing edge
    # Concatenate all the edges to form a list of edges
    edge_list = I + In + Out
    return edge_list

class Graph:
    def __init__(self, dataset, labeling_mode='spatial'):
        if dataset == 'h36m':
            num_node = 17
            ...
            num_node = 15
            self_link = [(i, i) for i in range(num_node)]
            inward_ori_index = [(15,2), (2,3), (3,4), (4,5), (2,6), (6,7), (7,8), (2,1), (1,12), (12,13),
                                (13,14), (1,9), (9,10), (10,11)]
        elif dataset =='3dhp':
            num_node = 17
            self_link = [(i, i) for i in range(num_node)]
            inward_ori_index = [(2,1),(3,2),(4,3),(5,4),(6,2),(7,6),(8,7),(16,2),(16,15),(15,12),(15,9),
                                (13,12),(14,13),(10,9),(11,10),(17,2)]
        else:
            raise ValueError('Dataset not found...')

        # Converts the edge list to a zero-indexed form
        inward = [(i - 1, j - 1) for (i, j) in inward_ori_index]
        outward = [(j, i) for (i, j) in inward]  # The transpose forms the exit edge

        self.num_node = num_node
        self.self_link = self_link
        self.inward = inward
        self.outward = outward
        self.neighbor = inward + outward  # All neighbors information

        self.edge_index = self.get_edge_index(labeling_mode)

    def get_edge_index(self, labeling_mode='spatial'):
        # Generate edge_index
        if labeling_mode is None:
            return self.edge_index
        if labeling_mode == 'spatial':
            edge_list = get_spatial_graph(self.self_link, self.inward, self.outward)
        else:
            raise ValueError("Invalid labeling mode")

        # Converts the edge list to edge_index
...
class TemporalCrossAttention(nn.Module):
    ...
    def __init__(self, latent_dim, text_latent_dim, num_head, dropout, time_embed_dim):
        ...
        self.gat = GATConv(latent_dim, 512, heads=1, dropout=dropout)
    def forward(self, x, xf, emb,edge_index):
        """
        x: B, T, D
        xf: B, N, L
        """
        ...
        x_gat=x_gat.view(B, T, D)
        y = x_gat + self.proj_out(y, emb)
...
class  MixSTE2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        # Sparse edge index and graph attention mechanism
        self.A = Graph(dataset=args.dataset)
        self.edge_index = self.A.edge_index
        self.STEgat = GATConv(512, 512, heads=1, dropout=drop_rate)
        ...
        self.remain_len = 4
        self.remain_len_subject = 7
        self.remain_len_verb = 12

        ctx_vectors_subject = torch.empty((7-self.remain_len_subject), 512, dtype=self.clip_text.dtype)
        nn.init.normal_(ctx_vectors_subject, std=0.02)
        self.ctx_subject = nn.Parameter(ctx_vectors_subject)

        ctx_vectors_verb = torch.empty((12-self.remain_len_verb), 512, dtype=self.clip_text.dtype)
    ...
    def STE_forward(self, x_2d, x_3d, t, xf_proj,edge_index):
    ...
    def encode_text(self, text, pre_text_tensor):
        ...
        learnable_prompt_subject = torch.cat((learnable_prompt_subject, pre_text_tensor[:, 0, :self.remain_len_subject, :]), dim=1)
        ...
        learnable_prompt_verb = torch.cat((learnable_prompt_verb, x[:, :self.remain_len_verb, :]), dim=1)
    ...
    def forward(self, x_2d, x_3d, t, text, pre_text_tensor):
        ...
        x, time_embed = self.STE_forward(x_2d, x_3d, t, xf_proj,edge_index=self.edge_index)

        x = self.temporal_cross_attn(x, xf_out, time_embed,edge_index=self.edge_index)
